#!/usr/bin/env python3
# ============================================================
# TOI (TESS) - Treinamento CPU a partir de dados pré-processados
# *** Versão com tratamento de desbalanceamento ***
...
# Conjunto completo esperado pelo pré-processador TOI (antes do VT).
TOI_FULL_FEATURES = [
    "pl_orbper",
    "pl_trandurh",
    "pl_trandep",
...
def assert_toi_features(feat_names, strict: bool = False):
    """
    strict=False (padrão): aceita SUBCONJUNTO das TOI_FULL_FEATURES (pós-VarianceThreshold).
    strict=True: exige que as features correspondam exatamente às TOI_FULL_FEATURES naquela ordem.
    """
    feats = list(map(str, feat_names))
    if strict:
        if len(feats) != len(TOI_FULL_FEATURES) or any(a != b for a, b in zip(feats, TOI_FULL_FEATURES)):
            raise ValueError(
                "As features do NPZ não batem com o conjunto fixo esperado de TOI.\n"
                f"Esperado (ordem exata): {TOI_FULL_FEATURES}\n"
                f"Encontrado: {feats}"
            )
    else:
        unknown = [c for c in feats if c not in TOI_FULL_FEATURES]
        if unknown:
            raise ValueError(
                "Foram encontradas features desconhecidas no NPZ em relação ao conjunto TOI esperado.\n"
                f"TOI esperado (superset): {TOI_FULL_FEATURES}\n"
                f"Encontrado: {feats}\n"
...
def hyperparam_spaces_rf_xgb(base_spw: float):
    # RF com busca em hiperparâmetros usuais
    rf_param_dist = {
    ...
    # XGB: além dos HPs usuais, busca scale_pos_weight ao redor do valor base (±50%)
    low = max(base_spw * 0.5, 1e-6)
    width = base_spw * 1.0  # uniform(low, width) -> [low, low+width] = [0.5*spw, 1.5*spw]
    xgb_param_dist = {
...
def main():
    parser = argparse.ArgumentParser(description="Treino TOI CPU (com tratamento de desbalanceamento) a partir de dados pré-processados")
    parser.add_argument("--data_npz", type=str, default=os.path.join("preprocessed", "data_ready.npz"),
                        help="Caminho para o NPZ gerado pelo toi_preprocess.py.")
    ...
    feats_checked = assert_toi_features(feat_names, strict=args.strict_features)
    ...
    rf_clf = RandomForestClassifier(
        ...
        n_jobs=-1,
        class_weight="balanced"  # <-- chave para desbalanceamento
    )
    xgb_clf = XGBClassifier(
        ...
        random_state=RANDOM_STATE,
        nthread=-1,
        scale_pos_weight=spw  # <-- valor inicial; também será otimizado na busca
    )
