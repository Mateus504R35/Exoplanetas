#!/usr/bin/env python3
# ============================================================
# TOI (TESS) - ETL/Pré-processamento -> dados prontos p/ treino
# Conjunto FIXO de features numéricas:
#   - pl_orbper   (Período orbital, dias)
#   - pl_trandurh (Duração do trânsito, horas)
...
#
# Rótulo (TARGET): tfopwg_disp -> {CP,KP}=1 ; {FP,FA}=0
#
...
TARGET_COL = "tfopwg_disp"   # CP/KP -> 1 ; FP/FA -> 0
POS_SET = {"CP", "KP"}
NEG_SET = {"FP", "FA"}
...
def clean_and_select(df_raw: pd.DataFrame) -> pd.DataFrame:
    ...
    if TARGET_COL not in df_raw.columns:
        raise ValueError(f"É necessário conter a coluna '{TARGET_COL}' no CSV TOI.")
    # Filtra classes válidas e binariza
...
def main():
    parser = argparse.ArgumentParser(description="Pré-processamento TOI (features fixas) -> dados prontos para treino")
    parser.add_argument("--csv", required=True, type=str, help="Caminho para o CSV bruto (TOI).")
    ...
    print("=== PRONTO (TOI - features fixas) ===")
